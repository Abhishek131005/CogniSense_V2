...
================================================================================
 ...
 What you see:
   ...
   • Live per-eye ratios (L_norm vs R_norm) to expose cancellation bugs.
...
def _gauge(canvas, nx, ny):
    ...
    cv2.putText(canvas, txt, (bar_x, bar_y - 12),
...
def run():
    ...
    with tracker:
        while True:
            ...
            # ── per-eye diagnosis ─────────────────────────────────────────
            lcx_px, _ = FaceMeshTracker.iris_centroid(lm, LIRS_INDICES_FULL, w, h)
            rcx_px, _ = FaceMeshTracker.iris_centroid(lm, RIRS_INDICES_FULL, w, h)
            l_inner_px = lm[L_INNER_CORNER].x * w
            r_inner_px = lm[R_INNER_CORNER].x * w

            l_w = max(abs(lm[L_OUTER_CORNER].x * w - l_inner_px), 5.0)
            r_w = max(abs(lm[R_OUTER_CORNER].x * w - r_inner_px), 5.0)

            l_ratio = (lcx_px - l_inner_px) / l_w - 0.5
            r_ratio = (rcx_px - r_inner_px) / r_w - 0.5

            diag_txt = (f"L_norm={l_ratio:+.3f}  R_norm={r_ratio:+.3f}  "
                        f"avg={(l_ratio + r_ratio) / 2:+.3f}")
            cv2.putText(canvas, diag_txt, (20, h - 140),
            ...
            l_crop, _ = _crop(LIRS_INDICES_FULL[0])
            ...
            cv2.putText(canvas,
                        ...
                        (20, h - 180), FONT, 0.65, (200, 200, 200), 2)
